...
class HelpCommand(Command):
    ...
    def build_card(self, message, attachment_actions, activity):
        ...
        # commandTexts = textwrap.dedent(commandTexts).strip()
        ...
        # bodyTitle = TextBlock(commandTexts, wrap=True, size=FontSize.SMALL, color=Colors.LIGHT)
        #
        # image = Image(
        #     url=self.bot_help_image,
        #     size=self.bot_help_image_size)
        #
        # header_column = Column(items=[heading, subtitle, bodyTitle], width=2)
        ...
        # card = AdaptiveCard(body=[ColumnSet(columns=[header_column, header_image_column])])
        return commandTexts
